...
function hashPassword(password) {
    ...
    return new Promise((resolve, reject) => {
        crypto_1.default.randomBytes(config.saltBytes, function (err, salt) {
            if (err) {
                throw err;
            }
            crypto_1.default.pbkdf2(password, salt, config.iterations, config.hashBytes, config.digest, function (err, hash) {
                if (err) {
                    reject(err);
                }
...
function verifyPassword(password, combined, callback) {
    ...
    combined = Buffer.from(combined, 'hex');
    ...
    crypto_1.default.pbkdf2(password, salt, iterations, hashBytes, config.digest, function (err, verify) {
        if (err) {
...
const pass = 'abc1234';
hashPassword(pass).then(res => { verifyPassword(pass, res, (err, isValid) => { console.log(isValid); }); });
